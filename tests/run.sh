...
# the loader falls back to <repo>/config-override.toml when the redirected
# XDG paths are empty: a developer's local override would leak into the
# "documented defaults" assertions. Move it aside for the run, restore on
# exit (it is gitignored and regenerated by convention, so a hard kill is
# recoverable).
if [[ -e config-override.toml ]]; then
    mv config-override.toml "$TMP/override-backup"
fi
trap '[[ ! -e "$TMP/override-backup" ]] || mv "$TMP/override-backup" config-override.toml; rm -rf "$TMP"' EXIT
