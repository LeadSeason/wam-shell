...
import Config, { surfaceOpacity } from "../config"
...
export function syncTuning(): boolean {
    // headless precompile (`wam install`/`update`) runs sessionless, so
    // surfaceOpacity() reads desktopSession "" and emits 1 even with blur
    // configured — the first real shell start rewrites this and forces a
    // recompile, so the cost is one opaque boot after install
    const body =
        ...
        `$ws-playing-pulse: ${Config.workspaces.playingPulseMs}ms;\n` +
        // the on/off is folded into the value, so the scss never branches:
        // 1 is a no-op alpha
