...
import { cpu, ram, gpu, gpuTemp, netDown, netUp, formatRate,
    cpuHist, ramHist, gpuHist, ramSize, vram, loadAvg } from "../../../lib/sysstats"
...
const BARS = 12
const short = (hist: Accessor<{ v: number }[]>) =>
    hist.as(h => h.slice(-BARS))
        ...
        <With value={gpu}>
            {(g) => g !== null &&
                <box spacing={8}>
                    ...
                    />
                    <Graph hist={short(gpuHist)} className="statGpu" height={12} hexpand={false} />
                </box>}
        </With>
        <label
            cssClasses={["statNet"]}
            label={createComputed([netDown, netUp],
